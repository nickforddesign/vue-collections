...
(function (global, factory) {
	...
	(global['vue-collections'] = factory());
...
function _traverse (val, seen) {
  ...
  if ((!isA && !isObject$1(val)) || !Object.isExtensible(val)) {
    return
  }
  if (val.__ob__) {
...
function _createElement (
  ...
  if (process.env.NODE_ENV !== 'production' &&
    isDef$1(data) && isDef$1(data.key) && !isPrimitive(data.key)
  ) {
    warn(
...
function applyNS (vnode, ns) {
  ...
  if (isDef$1(vnode.children)) {
    for (var i = 0, l = vnode.children.length; i < l; i++) {
      ...
      if (isDef$1(child.tag) && isUndef(child.ns)) {
        applyNS(child, ns);
      }
    }
  }
...
function normalizeEvents (on) {
  var event;
  /* istanbul ignore if */
  if (isDef$1(on[RANGE_TOKEN])) {
    // IE input[type=range] only supports `change` event
    event = isIE ? 'change' : 'input';
    on[event] = [].concat(on[RANGE_TOKEN], on[event] || []);
    delete on[RANGE_TOKEN];
  }
  if (isDef$1(on[CHECKBOX_RADIO_TOKEN])) {
...
function isInputChanged (elm, newVal) {
  ...
  if (isDef$1(modifiers) && modifiers.number) {
    return toNumber(value) !== toNumber(newVal)
  }
  if (isDef$1(modifiers) && modifiers.trim) {
...
function leave (vnode, rm) {
  ...
  /* istanbul ignore if */
  if (isDef$1(el._leaveCb) || el.nodeType !== 1) {
    return
  }
  ...
  if (process.env.NODE_ENV !== 'production' && isDef$1(explicitLeaveDuration)) {
...
function getRequestFunction(store) {
  var request = path_1(['$request'], store._vm);
  if (!request) {
...
var Store = function () {
  _createClass(Store, null, [{
    key: 'init',
    value: function init(Vue, _Vuex) {
      Vuex = process.env.NODE_ENV === 'test' ? require('vuex') : _Vuex;
    }
  }]);

  function Store(model) {
    var id_attribute = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '_id';
    var basePath = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
    var createPath = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';
    var reverse = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
    var sortBy = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;

    _classCallCheck(this, Store);

    var _basePath = function _basePath() {
      var output = void 0;
    ...
    return new Vuex.Store({
      ...
      mutations: {
        ...
        FETCH: function FETCH(state, collection) {
          state.collection = _process(collection);
        },
        ADD: function ADD(state, model) {
          if (reverse) {
        ...
        DELETE: function DELETE(state, id) {
          var i = state.collection.findIndex(function (model) {
            return model[id_attribute] === id;
          });
          state.collection.splice(i, 1);
        },
        UPDATE: function UPDATE(state, id, data) {
          var i = state.collection.findIndex(function (model) {
            return model[id_attribute] === id;
          });
          state.collection[i] = mergeDeepRight_1(state.collection[i], data);
      ...
      actions: {
        ...
        fetch: function fetch(_ref2) {
          ...
          var req = getRequestFunction(this)('' + _basePath());
          req.then(function (response) {
            commit('FETCH', response);
          });
          return req;
        },
        add: function add(_ref3, model) {
          var commit = _ref3.commit;

          var req = getRequestFunction(this)(_basePath() + '/' + createPath, {
            method: 'POST',
            body: model
          });
          req.then(function (response) {
        ...
        delete: function _delete(_ref4, id) {
          ...
          var req = getRequestFunction(this)(_basePath() + '/' + id, {
            method: 'DELETE'
          });
          req.then(function (response) {
            commit('DELETE', id);
...
var Collection = function () {
  _createClass(Collection, null, [{
    key: 'init',
    value: function init(Vue$$1, Vuex) {
      Store.init(Vue$$1, Vuex);
  ...
  function Collection(_ref) {
    ...
    var store = new Store(model, id_attribute, basePath, createPath, reverse, sortBy);
    return new Vue$3({
      name: 'collection',
      store: store,
      computed: {
...
var VueCollection = function () {
  ...
  _createClass(VueCollection, null, [{
    key: 'install',
    value: function install(Vue, Vuex) {
    ...
    value: function Collection$$1() {
      ...
      return new (Function.prototype.bind.apply(Collection, [null].concat(args)))();
    }
  }]);
